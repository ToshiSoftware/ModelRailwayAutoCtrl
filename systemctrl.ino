...
void GetSpeedVr( void )
{
  ...
  int def;
  // Analog in
  potValue = analogRead( PIN_ANALOG_VR );
  gbTrain.tempSpeed = potValue/16;
  gbTrain.tempSpeed = gbTrain.tempSpeed * gbTrain.maxSpeed / 255;
  if(gbTrain.tempSpeed<TRAIN_SPEED_TOLERANCE)   gbTrain.tempSpeed=0;
  if(gbTrain.tempSpeed>255-TRAIN_SPEED_TOLERANCE) gbTrain.tempSpeed=255;
...
void updateEncoderValue(void)
{
    ...
    if(gbEncoder[0].rotation_dir == ROTATION_PLUS){
      ...
      if(gbSystem.scenario_number >= NUM_SCENARIO) gbSystem.scenario_number=0;
    }
    else if(gbEncoder[0].rotation_dir == ROTATION_MINUS){
      gbEncoder[0].rotation_dir = ROTATION_STOP;
      gbSystem.scenario_number--;
      if(gbSystem.scenario_number < 0) gbSystem.scenario_number=NUM_SCENARIO;
    }
}
